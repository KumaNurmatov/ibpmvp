...
function BuyerHome({ nav, role }) {
  return (
    <div style={{
      ...
      background:`radial-gradient(120% 80% at 50% -10%, #14181C 0%, ${DS.bg} 65%)`,
      paddingTop:58, overflow:'hidden',
    }}>
      <HomeHeaderV3 onProfile={()=>nav('profile')}/>

      {/* Greeting */}
      <div style={{padding:'12px 20px 12px', flexShrink:0}}>
        <div style={{fontSize:11, color:DS.muted, letterSpacing:'0.14em', textTransform:'uppercase', fontWeight:700, marginBottom:3}}>
          Добрый день
        </div>
        <div style={{fontSize:21, fontWeight:800, color:DS.text, letterSpacing:-0.5, lineHeight:1.15, marginBottom:4}}>
          Что закупаем сегодня?
        </div>
        <div style={{fontSize:12, color:DS.sub, letterSpacing:-0.05}}>
          <span style={{fontFamily:'"SF Mono",ui-monospace,monospace', fontSize:11}}>CN → RU</span>
          <span style={{margin:'0 7px', color:DS.faint}}>·</span>
          384 проверенных поставщика онлайн
        </div>
      </div>
      ...
      {/* Chip rail */}
      <div style={{padding:'2px 0 0', flexShrink:0, display:'flex', gap:7, overflowX:'auto', paddingLeft:20, paddingRight:20, WebkitMaskImage:'linear-gradient(to right, #000 88%, transparent 100%)'}}>
        {['Все','Проверенные','Готово к отправке','MOQ < 50','Металл','Текстиль'].map((l,i)=>(
          <FilterChip key={l} label={l} active={i===0}/>
        ))}
      </div>

      {/* Categories — scrollable; takes remaining space above AI panel */}
      <div style={{padding:'14px 20px 10px', flex:1, overflowY:'auto', overflowX:'hidden', minHeight:0}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:9}}>
          <div style={{fontSize:11, fontWeight:700, color:DS.muted, letterSpacing:'0.14em', textTransform:'uppercase'}}>Категории</div>
          <div onClick={()=>nav('catalog')} style={{fontSize:11.5, color:'var(--accent)', fontWeight:600, cursor:'pointer'}}>Все →</div>
        </div>
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:9, marginBottom:9}}>
          {V3_CATS.slice(0,2).map(c =>
            <CatCard key={c.id} cat={c} big onPress={()=>nav('catalog',{cat:c.label})}/>
          )}
        </div>
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:9, marginBottom:9}}>
          {V3_CATS.slice(2,4).map(c =>
            <CatCard key={c.id} cat={c} onPress={()=>nav('catalog',{cat:c.label})}/>
          )}
        </div>
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:9}}>
          {V3_CATS.slice(4).map(c =>
            <CatCard key={c.id} cat={c} onPress={()=>nav('catalog',{cat:c.label})}/>
          )}
        </div>
      </div>

      <AIPanelV3 onAI={()=>nav('ai')} onAIMsg={(t)=>nav('ai',{msg:t})}/>
